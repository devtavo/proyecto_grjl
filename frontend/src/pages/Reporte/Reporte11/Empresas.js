...
export default function Empresas({ reporteId, handleClickEmpresa }) {
        ...
        const getEmpresas = async () => {
            ...
            const total =
                [{
                    ...
                    idEtt: '',
                    razonSocialEmpresa: '',
                    tiempoSinTransmisionGps: ''+empresas.data.resumen[0].tiempo
                }];
            const concatEmpresas = [...empresas.data.detalle, ...total];
            // empresas.data.resumen[0].tiempo = getTotals(empresas.data.detalle, 'tiempoSinTransmisionGps');
    ...
    const columnsDet = [
        {
            Header: 'Tiempo sin transmisión del GPS durante la prestación del Servicio',
            columns: [
                {
                    Header: '#',
                    accessor: 'id',
