...
export default function Rutas({ consultaId, empresaId, rutaId, inicio, final, codigoRuta, razonSocial, setShowVehiculos }) {
        ...
        const getVehiculos = async () => {
            ...
            const total =
                [{
                    ...
                    idEtt: '',
                    razonSocialEmpresa: '',
                    cantidad: getTotals(vehiculos.data.detalle, 'cantidad'),
            ...
            const categories = vehiculos.data.detalle.reduce((acc, { codigoRuta }) => (acc.push(codigoRuta), acc), []);
            const cantidad = vehiculos.data.detalle.reduce((acc, { cantidad }) => (acc.push(parseInt(cantidad)), acc), []);
            ...
            chartOptions.title.text = `KILÓMETROS RECORRIDOS POR LOS VEHÍCULOS EN SERVICIO DE LA RUTA ${codigoRuta} DE LA EMPRESA ${razonSocial} desde el ${inicio} al ${final}`;
    ...
    const columnsDet = [
        {
            Header: `KILÓMETROS RECORRIDOS POR LOS VEHÍCULOS EN SERVICIO  DE LA RUTA ${codigoRuta} DE LA EMPRESA ${razonSocial} DESDEL EL ${inicio} al ${final}`,
            columns: [
                {
                    Header: '#',
                    accessor: 'id',
