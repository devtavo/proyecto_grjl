...
export const options = {
    ...
    title: {
        text: 'Promedio de tiempos de vehiculos dentro de la ruta'
    },
    xAxis: [{
        categories: [],
        crosshair: true
    ...
    series: [
        {
            name: 'Promedio de tiempo de vehiculos',
            type: 'spline',
            data: [],
            tooltip: {
                valueSuffix: ' '
...
export default function Empresas({ consultaId, handleClickEmpresa }) {
    ...
    useEffect(() => {
        const getEmpresas = async () => {
            ...
            const vehiculos = empresas.data.detalle.reduce((acc, { vehiculos }) => (acc.push(parseInt(vehiculos)), acc), []);
            const tiempoAcuTransmisionGps = empresas.data.detalle.reduce((acc, { tiempoAcuTransmisionGps }) => (acc.push(parseInt(tiempoAcuTransmisionGps)), acc), []);

            chartOptions.title.text = `Promedio De Tiempos Entre Paraderos Y Ruta Completadas Por Empresa`;
            chartOptions.xAxis[0].categories = categories;
            chartOptions.series[0].data = tiempoAcuTransmisionGps;
            chartOptions.series[1].data = vehiculos;
    ...
    const columnsDet = [
        {
            Header: 'Promedio de tiempos De Recorrido Entre Paraderos Y Rutas Completadas Por Vehículo',
            columns: [
                {
                    Header: '#',
                    accessor: 'id',
                ...
                {
                    Header: 'Promedio de tiempo vehiculos en la ruta (días y horas)',
                    accessor: 'tiempoAcuTransmisionGps',
                    alignBody: 'center',
                    alignHeader: 'center'
                },
                {
                    Header: 'Total Vehiculos',
                    accessor: 'vehiculos',
                    alignBody: 'center',
                    alignHeader: 'center'
                },
