...
export default function Rutas({ consultaId, empresaId, razonSocial, inicio, final, handleClickRuta }) {
    ...
    useEffect(() => {
        const getRutas = async () => {
            ...
            chartOptions.title.text = `Ranking De Kilometros Recorridos Por Vehículos En Servicio de la empresa ${razonSocial} desde el ${inicio} al ${final}`;
    ...
    const columnsDet = [
        {
            Header: `Ranking De Kilometros Recorridos Por Vehículos En Servicio de la empresa ${razonSocial} desde el ${inicio} al ${final}`,
            columns: [
                {
                    Header: '#',
                    accessor: 'id',
