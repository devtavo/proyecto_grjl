...
export default function Vehiculos({ consultaId, empresaId, rutaId, inicio, final, codigoRuta, razonSocial, setShowVehiculos }) {
    ...
    useEffect(() => {
        const getVehiculos = async () => {
            ...
            chartOptions.title.text = `PROMEDIO DE KILÓMETROS RECORRIDOS POR LOS VEHÍCULOS EN SERVICIO DE LA RUTA ${codigoRuta} DE LA EMPRESA ${razonSocial} desde el ${inicio} al ${final}`;
    ...
    const columnsDet = [
        {
            Header: `PROMEDIO DE KILÓMETROS RECORRIDOS POR LOS VEHÍCULOS EN SERVICIO  DE LA RUTA ${codigoRuta} DE LA EMPRESA ${razonSocial} DESDEL EL ${inicio} al ${final}`,
            columns: [
                {
                    Header: '#',
                    accessor: 'id',
