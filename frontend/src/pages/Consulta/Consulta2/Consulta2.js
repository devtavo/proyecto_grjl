...
export default function Consulta2({ consultaId }) {
    ...
    if (vista === VISTA_EMPRESAS)
        return (
            <>
                <BackButton to='/consultas' bread={[{ name: 'Inicio', path: '../' }, { name: 'Supervision', path: '.' }, { name: 'Consultas', path: '../consultas' }, { name: 'Nivel Empresas', path: '.' }]} />
                <Navigation
                    title="Reporte de Constancias emitidas"
    ...
    if (vista === VISTA_RUTAS)
        return (
            <>
                <BackButton to='/consultas' bread={[{ name: 'Inicio', path: '../' }, { name: 'Supervision', path: '.' }, { name: 'Consultas', path: '../consultas' }, { name: 'Nivel Empresas', path: '.' },{ name: 'Nivel Rutas', path: '.' }]} />
                <Navigation
                    title="Vehículos en servicio vs vehículos autorizados"
                    breadcrumb={breadCrumb}
                />
                {/* <Breadcrumb 
                    title='Nivel Rutas'
