...
export default function Rutas({ consultaId, empresaId, razonSocial, inicio, final, handleClickRuta }) {
    ...
    useEffect(() => {
        const getRutas = async () => {
            ...
            chartOptions.title.text = `CANTIDAD DE KILÓMETROS RECORRIDOS POR LOS VEHÍCULOS EN SERVICIO DE LA EMPRESA ${razonSocial} desde el ${inicio} al ${final}`;
    ...
    const columnsDet = [
        {
            Header: `CANTIDAD KILÓMETROS RECORRIDOS POR LOS VEHÍCULOS EN SERVICIO DE LA EMPRESA ${razonSocial} desde el ${inicio} al ${final}`,
            columns: [
                {
                    Header: '#',
                    accessor: 'id',
